...
public class CompetitionDijkstra {
    ...
    */
    CompetitionDijkstra (String filename, int speedA, int speedB, int speedC){
    	...
    	try
    	{
        	...
        	while(scanner.hasNextLine())
        	{
        		...
        		if(i==0) //Will only enter on first pass, create double arrays of appropriate size here for distTo and edgeTo and give them initial values
        		{
        			distTo = new double[Integer.parseInt(split[i])][Integer.parseInt(split[i])]; //Need to parse from the split strings
            		edgeTo = new int[Integer.parseInt(split[i])][Integer.parseInt(split[i])]; 
    ...
    public int timeRequiredforCompetition(){
        ...
        //TO DO
        double maxDistance = 0.0; //In Km
        if(minSpeed <= 0 || maxDistance == 0)
    	{
    		return - 1;
    	}
        double maxDistanceInM = maxDistance*1000;
        int timeReq = (int) Math.ceil((maxDistanceInM)/minSpeed); //Need to use Math.Ceil to avoid rounding errors caused by converting double to int
        return timeReq;
    }
